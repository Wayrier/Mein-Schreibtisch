...
// =======================================
// delete_appointment_ajax.php
// Zweck: Eigenen Termin via AJAX loeschen
// =======================================
...
// AJAX-Endpunkt fuer appointments.php.
// Gibt JSON zurueck, damit die Seite ohne Reload aktualisiert werden kann.
...
try {
    // Vor dem Loeschen merken, welche Dateien an diesem Termin haengen.
    $files_for_cleanup = collect_appointment_files_for_cleanup($pdo, $user_id, $id);

    $pdo->beginTransaction();

    // Loescht nur Termine des eingeloggten Users.
    $stmt = $pdo->prepare("
    ...
    if ($deleted) {
        // Conversion-Historie zum Termin entfernen, aber keine Zielobjekte loeschen.
        $stmt = $pdo->prepare("
        ...
        // Dateien nur entfernen, wenn sie nach dem Termin-Loeschen nirgendwo mehr benutzt werden.
        $paths_to_delete = delete_unlinked_file_records($pdo, $user_id, $files_for_cleanup);
    }

    $pdo->commit();

    // Dateisystem erst nach erfolgreichem Commit anfassen.
